...
-- VERSIÓN 3: Corregido tipo de datos auth.uid() vs users.id
-- =====================================================

-- =====================================================
-- NOTA IMPORTANTE SOBRE TIPOS DE DATOS:
-- auth.uid() devuelve UUID
-- Si users.id es TEXT, necesitamos cast: auth.uid()::text
-- Si users.id es UUID, necesitamos cast: auth.uid()
-- =====================================================
...
-- NOTAS IMPORTANTES
-- =====================================================
-- 1. auth.uid() devuelve UUID, se convierte a text con ::text
-- 2. Si users.id es UUID en lugar de TEXT, cambiar auth.uid()::text por auth.uid()
-- 3. Las columnas camelCase deben ir entre comillas dobles: "tenantId"
-- 4. order_items no tiene tenantId, usa orderId para join con orders
