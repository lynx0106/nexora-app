...
export class AutomationsScheduler {
  ...
  async handleCron() {
    if (this.isProcessing) {
      this.logger.debug('Previous automation run still in progress, skipping...');
      return;
    }
    ...
    try {
      ...
      if (dueAutomations.length > 0) {
        this.logger.log(`Found ${dueAutomations.length} due automations to execute`);
        ...
        for (const automation of dueAutomations) {
          ...
          try {
            ...
            await this.automationsService.executeAutomation(automation, savedRun);
          } catch (error) {
      ...
      this.logger.error(`Error in automation scheduler: ${error.message}`, error.stack);
    } finally {
