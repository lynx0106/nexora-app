...
export class UsersService {
  ...
  async seedSuperAdmin() {
    ...
    const password = process.env.SUPERADMIN_PASSWORD || this.generateSecurePassword();
    const passwordHash = await bcrypt.hash(password, 10);
    
    // Log warning if using default/autogenerated password
    if (!process.env.SUPERADMIN_PASSWORD) {
      this.logger.warn(
        'SUPERADMIN_PASSWORD environment variable not set. ' +
        'Using auto-generated password. Check logs for details.'
      );
      // Only log the password on first creation
      if (!existing) {
        this.logger.log(`Superadmin temporary password: ${password}`);
        this.logger.log('Please change this password immediately after first login!');
      }
    }
  ...
  async seedClients(tenantId: string) {
    ...
    for (const client of clients) {
      try {
        ...
        // Generate random password for demo client
        const tempPassword = this.generateSecurePassword();
        const passwordHash = await bcrypt.hash(tempPassword, 10);
        this.logger.log(`Demo client ${client.email} created with temporary password (check logs if needed)`);
        const user = this.usersRepository.create({
  ...
  async seedDemoUsers() {
    ...
    const demoPassword = this.generateSecurePassword();
    this.logger.warn('Demo users seeded with auto-generated passwords - NOT FOR PRODUCTION');
    const demoUsers = [
      {
        firstName: 'Carlos',
        lastName: 'Lince',
