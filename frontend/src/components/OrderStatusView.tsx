...
export function OrderStatusView({ order }: OrderStatusViewProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "paid":
        return "bg-emerald-900/40 text-emerald-200 border border-emerald-700";
      case "pending":
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case "paid":
        return t("public.status_paid");
      case "pending":
        return t("public.status_pending");
      case "cancelled":
        return t("public.status_cancelled");
      case "refunded":
        return t("public.status_refunded");
      default:
        return status;
    }
  };
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-slate-400">
            {t("public.order_details", { id: order.id.slice(0, 8) })}
          </p>
        </div>
            ...
            <div className="px-4 py-5 sm:px-6 bg-amber-900/20 border-b border-amber-800">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center">
                  <svg
                    className="h-6 w-6 text-amber-300 mr-3"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
